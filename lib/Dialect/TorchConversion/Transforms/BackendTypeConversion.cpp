...
void mlir::torch::TorchConversion::setupBackendTypeConversionForTosa(
    ConversionTarget &target, TypeConverter &typeConverter) {
  auto valueTensorTypeConversion =
      [](Torch::ValueTensorType type) -> std::optional<Type> {
    auto builtinType = type.toBuiltinTensor();
    ...
    // Convert signed integer types to signless, but preserve unsigned types.
    // TOSA ops like tosa.clamp need unsigned type info for correct bounds.
